...
export default function PublicationReview({
  ...
  const handleApprove = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(error.error || 'Failed to approve publication')
      }
      ...
      alert(error instanceof Error ? error.message : 'Failed to approve publication')
    } finally {
      setIsLoading(false)
    }
  }
  ...
  const handleReject = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        const error = await response.json()
        throw new Error(error.error || 'Failed to reject publication')
      }

      alert('Publication rejected. Creator has been notified.')
      router.push('/admin/content')
      router.refresh()
    } catch (error) {
      console.error('Error rejecting publication:', error)
      alert(error instanceof Error ? error.message : 'Failed to reject publication')
    } finally {
      setIsLoading(false)
      ...
      <div className="bg-white shadow rounded-lg p-6 mb-6">
        ...
        <div className="space-y-3">
          <label className="flex items-center">
            <input
              type="checkbox"
              checked={qualityChecks.pdfValid}
              onChange={(e) =>
                setQualityChecks({ ...qualityChecks, pdfValid: e.target.checked })
              }
          ...
          <label className="flex items-center">
            <input
              type="checkbox"
              checked={qualityChecks.contentAppropriate}
              onChange={(e) =>
                setQualityChecks({ ...qualityChecks, contentAppropriate: e.target.checked })
              }
              className="h-4 w-4 text-blue-600 focus:ring-blue-500 border-gray-300 rounded"
            />
            <span className="ml-3 text-sm text-gray-700">
          ...
          <label className="flex items-center">
            <input
              type="checkbox"
              checked={qualityChecks.metadataAccurate}
              onChange={(e) =>
                setQualityChecks({ ...qualityChecks, metadataAccurate: e.target.checked })
              }
              className="h-4 w-4 text-blue-600 focus:ring-blue-500 border-gray-300 rounded"
            />
            <span className="ml-3 text-sm text-gray-700">
              Title, description, and keywords are accurate
            </span>
          </label>
          <label className="flex items-center">
            <input
              type="checkbox"
              checked={qualityChecks.doiValid}
              onChange={(e) =>
                setQualityChecks({ ...qualityChecks, doiValid: e.target.checked })
              }
              className="h-4 w-4 text-blue-600 focus:ring-blue-500 border-gray-300 rounded"
            />
            <span className="ml-3 text-sm text-gray-700">
              DOI is valid (if provided) or N/A
            </span>
          </label>
        </div>
      </div>
      ...
      {showRejectModal && (
        <div className="fixed inset-0 bg-gray-500 bg-opacity-75 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg p-6 max-w-md w-full mx-4">
            <h3 className="text-lg font-medium text-gray-900 mb-4">Reject Publication</h3>
            <p className="text-sm text-gray-500 mb-4">
              Please provide detailed feedback for &quot;{publicationTitle}&quot;. This will be sent
              to the creator.
            </p>
            <textarea
